...
function resolveOptions(options, root) {
  ...
  if (resolved.globs) {
    resolved.globs = toArray(resolved.globs).map((glob) => slash2(resolveGlobsExclude(root, glob)).replace(/\[/g, "\\[").replace(/\]/g, "\\]"));
    ...
    resolved.globs = resolved.resolvedDirs.map(
      (i) => (resolved.deep ? slash2(join(i, `**/*.${extsGlob}`)) : slash2(join(i, `*.${extsGlob}`))).replace(/\[/g, "\\[").replace(/\]/g, "\\]")
    );
